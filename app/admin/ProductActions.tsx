  ...
  async function handleAddVariacao() {
    ...
    try {
      ...
      const { data: inserted, error: insertError } = await supabase
        ...
        .select().single()
      if (insertError || !inserted) throw new Error('Erro ao registrar variaÃ§Ã£o.')
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">
                    TÃ­tulo <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">DescriÃ§Ã£o</label>
                  <textarea
                    value={editDescricao}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">
                    PreÃ§o (R$) <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
            ...
            {tab === 'fotos' && (
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-400 text-center">A primeira foto Ã© a capa. MÃ¡ximo 6 imagens.</p>
              </div>
            )}

            {/* â”€â”€ Aba Cores / VariaÃ§Ãµes â”€â”€ */}
            {tab === 'cores' && (
              <div className="space-y-4">
                ...
                <div className="border border-rose-100 rounded-xl p-3 space-y-3">
                  <p className="text-xs font-semibold text-gray-500 flex items-center gap-1.5">
                    <Palette size={13} /> Nova cor / variaÃ§Ã£o
                  </p>
                  <input
                    type="text"
                  ...
                  <div className="flex items-center gap-3">
                    <label className="flex-1 flex items-center justify-center gap-1.5 border-2 border-dashed border-rose-200 rounded-xl py-2 text-xs text-rose-400 font-semibold cursor-pointer hover:bg-rose-50 transition-colors">
                      <ImagePlus size={14} />
                      {novaVarFile ? novaVarFile.name.slice(0, 16) + 'â€¦' : 'Selecionar imagem'}
